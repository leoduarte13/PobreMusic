...
export default function App() {
  ...
  useEffect(()=>{
    const init=()=>{ if(!window.YT?.Player)return; playerRef.current=new window.YT.Player('yt-engine',{height:'1',width:'1',playerVars:{autoplay:0,controls:0,playsinline:1,rel:0,iv_load_policy:3,enablejsapi:1,origin:location.origin},events:{
    ...
    if(window.YT?.Player)init(); else {const s=document.createElement('script');s.id='youtube-api';s.src='https://www.youtube.com/iframe_api';document.head.appendChild(s);window.onYouTubeIframeAPIReady=init;}
  ...
  useEffect(()=>{if('mediaSession' in navigator){const ms=navigator.mediaSession;ms.setActionHandler('play',()=>{intentRef.current=true;playerRef.current?.playVideo?.()});ms.setActionHandler('pause',()=>{intentRef.current=false;playerRef.current?.pauseVideo?.()});ms.setActionHandler('nexttrack',nextTrack);ms.setActionHandler('previoustrack',prevTrack);ms.setActionHandler('seekbackward',()=>playerRef.current?.seekTo?.(Math.max(0,time-10),true));ms.setActionHandler('seekforward',()=>playerRef.current?.seekTo?.(Math.min(duration,time+10),true));}},[nextTrack,prevTrack,time,duration]);
  useEffect(()=>{const wake=()=>{if(intentRef.current && document.visibilityState==='visible'){try{playerRef.current?.playVideo?.()}catch{}}};document.addEventListener('visibilitychange',wake);return()=>document.removeEventListener('visibilitychange',wake)},[]);
  ...
  const importPlaylist=async()=>{
    ...
    try{if(!parseSpotifyId(url))throw new Error('Cole o link de uma playlist pública do Spotify.');const r=await fetch(`/api/public-playlist?url=${encodeURIComponent(url)}`,{cache:'no-store'});const d:PlaylistData=await r.json();if(!r.ok||!d.sucesso)throw new Error(d.error||'Não foi possível importar a playlist.');setPlaylist(d);
      ...
      setTracks(resolved);
      const first=resolved.findIndex(t=>!!t.videoId);if(first>=0)playIndex(first,false);
  ...
  const changeVolume=(v:number)=>{setVolume(v);setMuted(v===0);playerRef.current?.setVolume?.(v)};
  const seek=(v:number)=>{setTime(v);playerRef.current?.seekTo?.(v,true)};
