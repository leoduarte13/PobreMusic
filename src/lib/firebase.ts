...
export function handleFirestoreError(error: unknown, operationType: OperationType, path: string | null): never {
  throw new Error(JSON.stringify({ error:error instanceof Error?error.message:String(error), authInfo:{userId:auth.currentUser?.uid,email:auth.currentUser?.email,emailVerified:auth.currentUser?.emailVerified,isAnonymous:auth.currentUser?.isAnonymous,providerInfo:auth.currentUser?.providerData||[]}, operationType, path }));
}
...
export function subscribeToAuth(callback:(user:GoogleUserProfile|null)=>void){
  try{const stored=localStorage.getItem("spottube_google_user_profile");if(stored)callback(JSON.parse(stored));}catch{}
  return onAuthStateChanged(auth,user=>{if(user&&!user.isAnonymous){persistGoogleUser(user).then(callback).catch(()=>callback({uid:user.uid,displayName:user.displayName,email:user.email,photoURL:user.photoURL}));}else if(!user&&!localStorage.getItem("spottube_google_user_profile")){callback(null);}});
}

export async function saveUserPlaylistToCloud(userId:string,p:SavedPlaylist){const id=auth.currentUser?.uid||userId;try{await setDoc(doc(db,"users",id,"playlists",p.id),{...p,userId:id,name:p.name.slice(0,150),description:(p.description||"").slice(0,500),cover:(p.cover||"").slice(0,2000),tracks:(p.tracks||[]).slice(0,500),updatedAt:Date.now(),isCloud:true},{merge:true});}catch(e){handleFirestoreError(e,OperationType.WRITE,`users/${id}/playlists/${p.id}`)}}
